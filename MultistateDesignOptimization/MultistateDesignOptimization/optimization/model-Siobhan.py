...
class Model:
	...
	"""
	# TODO: currently, the Model class probably wouldn't work is ensemble size is variable
	...
	selectedMicrostates = numpy.array(0);			# double[position][residue energy][macrostate][microstate], subset of microstateREsidueEnergies
	...
	def __init__(self, macrostates:enum, ensembleSize:int, backrubTemp:float, boltzmannTemp:float, weights:numpy.array, steepness:float, positions:int, positionOffset:int, useMicrostateData:bool = False, useAltAverageMethod:bool = False):
		"""
		Default constructor
		...
		if self.useMicrostateData:
			...
			self.microstateResidueEnergies = numpy.zeros([self.nPositions, 20, self.nMacrostates, 700], dtype = float); # magic number 700 - max expected number of microstates		
	...
	def constructFromExisting(existing, ensembleSize:int, backrubTemp:float, boltzmannTemp:float, weights:numpy.array, steepness:float):
		...
		if not existing.useMicrostateData and ensembleSize == existing.ensembleSize:
	...
	def addMicrostateData(self, macrostate:int, position:int, energies:"float") -> None:
		...
		for i in range(20):
			self.microstateResidueEnergies[position][i][macrostate][self.microstateCounts[position][macrostate]];
	...
	def averageMicrostates(self) -> None:
		...
		if not self.areMicrostatesPicked:
			...
			self.selectedMicrostates = numpy.zeros([self.nPositions, 20, self.nMacrostates, self.ensembleSize]);
			for i in range(self.nMacrostates):
				for j in range(self.ensembleSize):
					for k in range(self.nPositions):
						for l in range(20):
							self.selectedMicrostates[k][l][i][j] = self.microstateResidueEnergies[k][l][i][self.microstatesUsed[i][j]];

			self.areMicrostatesPicked = True;

		if not self.useAltAveragingMethod:
			if (temp == 0.0):
				self.macrostateResidueEnergies = numpy.amin(self.selectedMicrostates, axis = 3);
			elif (temp == -1.0):
				self.macrostateResidueEnergies = numpy.mean(self.selectedMicrostates, axis = 3); 
			else:
				self.macrostateResidueEnergies = numpy.sum(self.selectedMicrostates * numpy.exp(self.selectedMicrostates / -temp), axis = 3) / numpy.sum(numpy.exp(self.selectedMicrostates / -temp), axis = 3);
		else:
			if (temp == 0.0):
				self.macrostateResidueEnergies = numpy.amin(self.selectedMicrostates, axis = 3);
			elif (temp == -1.0):
				...
				self.macrostateResidueEnergies = -numpy.log(sum(numpy.exp(self.selectedMicrostates / -temp), axis = 3));
